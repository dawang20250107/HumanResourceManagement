...
const siliconValleyLoops = [
  ['Loop 21', 'Workflow Automation Studio', '可视化编排 18 条自动化', '触发器、条件、审批、通知与 Webhook 全链路编排'],
  ['Loop 22', 'Realtime Collaboration', '在线协同 326 人', '评论、@提及、任务分派、共享视图与操作留痕'],
  ['Loop 23', 'People Analytics Notebook', '分析模板 42 个', '供需、毛利、留存、薪酬公平可复用分析'],
  ['Loop 24', 'Scenario Sandbox', '沙盘模拟 128 次', '大促、缺勤潮、涨薪、客户砍单可回放推演'],
  ['Loop 25', 'Feature Flag & Entitlement', '租户能力包 16 个', '按客户、行业、地区灰度发布与权限售卖'],
  ['Loop 26', 'App Marketplace', '连接器 38 个', '电子签、税务、财务、考勤、BI、IM 生态集成'],
  ['Loop 27', 'Data Governance Hub', '血缘覆盖 97%', '指标口径、数据质量、PII 脱敏、保留策略统一治理'],
  ['Loop 28', 'Self-service BI', '自助报表 216 张', '业务用户自然语言生成图表与订阅报告'],
  ['Loop 29', 'Customer Success Cockpit', '健康分 86', '续约风险、价值实现、工单 SLA 与 QBR 自动化'],
  ['Loop 30', 'Revenue & Packaging Ops', 'ARR 模拟 ¥18.6M', '版本、席位、用量、模块包与毛利联动定价']
];

const masteryLoops = [
  ['Loop 16', '劳动力预测引擎', '未来 14 天缺口 312', '按城市、岗位、客户 SLA 预测供需缺口'],
  ['Loop 17', '移动员工自助', '今日自助完成 8,426 次', '抢单、换班、证照、工资单、培训一站式'],
  ['Loop 18', '供应商与外包伙伴', '伙伴履约 96.8%', '渠道报价、交付质量、风险评分统一管理'],
  ['Loop 19', '薪酬公平与预算护栏', '异常差异 12 条', '同岗同酬、补贴、加班、预算红线实时校验'],
  ['Loop 20', '重大事件应急指挥', '响应 SLA 12 分钟', '缺勤潮、设备故障、客户投诉一键建战情室']
];
...
function renderModule(key) {
  ...
  content.innerHTML = `
    ...
    <div class="mastery-loop-grid">
      ${masteryLoops.map(loop => `<button type="button" class="mastery-loop-card" data-mastery-loop="${loop[1]}"><span>${loop[0]}</span><strong>${loop[1]}</strong><p>${loop[2]}</p><em>${loop[3]}</em></button>`).join('')}
    </div>
    <div class="sv-loop-grid">
      ${siliconValleyLoops.map(loop => `<button type="button" class="sv-loop-card" data-sv-loop="${loop[1]}"><span>${loop[0]}</span><strong>${loop[1]}</strong><b>${loop[2]}</b><em>${loop[3]}</em></button>`).join('')}
    </div>
...
content.addEventListener('click', event => {
  ...
  if (event.target.closest('.mastery-loop-card')) {
    const name = event.target.closest('.mastery-loop-card').dataset.masteryLoop;
    document.querySelector('#insightDrawer').classList.add('open');
    document.querySelector('#insightDrawer').innerHTML = `<span>顶级实战能力</span><strong>${name}</strong><p>已接入预测、自助、伙伴、薪酬公平与应急指挥数据，支持从经营异常直接生成跨团队行动计划。</p>`;
    appendAudit(`启动顶级实战能力「${name}」`);
  }
  if (event.target.closest('.sv-loop-card')) {
    const name = event.target.closest('.sv-loop-card').dataset.svLoop;
    document.querySelector('#insightDrawer').classList.add('open');
    document.querySelector('#insightDrawer').innerHTML = `<span>Silicon Valley SaaS 能力</span><strong>${name}</strong><p>已进入平台化、生态化、数据治理、商业化运营视角，支持像顶尖 SaaS 一样持续扩展与规模化交付。</p>`;
    appendAudit(`打开硅谷 SaaS 能力「${name}」`);
  }
});
